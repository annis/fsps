...
29   CONTINUE
    ...
    IF (pset%sfh.EQ.2.OR.pset%sfh.EQ.3) THEN
     ...
     ELSE
        ...
        IF (write_compsp.EQ.4) &
             ...
             pset%tburst,pset%sf_start,pset%dust1,pset%dust2
        IF (write_compsp.EQ.1.AND.pset%sfh.EQ.5) &
             WRITE(10,35) tau,pset%sf_start,&
	     pset%sf_trunc,pset%sf_theta,pset%dust1,pset%dust2
     ENDIF
     IF (write_compsp.EQ.1.OR.write_compsp.EQ.3) THEN 
        WRITE(10,'("#")') 
        WRITE(10,32) 
  ...
  !-------------------------------------------------------------!
   ...
   DO i=imin,imax
	 ...
	 ! JTA
         CALL SAVE_COMPSP(write_compsp,ocompsp(i),zred,time_full(i),&
...
32 FORMAT('#   z log(age) log(mass) Log(lbol) log(SFR) mags (see FILTER_LIST)')
...
35 FORMAT('#   SFH 5: tau= ',F6.3,&
        ' Gyr, sf_start= ',F6.3,' Gyr, sf_trunc= ',F6.3, &
        ' Gyr, sf_theta= 'F6.3,', dust=(',F6.2,','F6.2,')')
...
SUBROUTINE SAVE_COMPSP(write_compsp,cspo,zred, time,mass,&
  ...
  REAL(SP), INTENT(in)    :: zred,time,mass,lbol,sfr,mdust
  ...
  fmt = '(F6.3,1x,F7.4,1x,3(F8.4,1x),000(F7.3,1x))'
  WRITE(fmt(29:31),'(I3,1x,I4)') nbands
  ! JTA
  ...
  !write to mags file
  IF (write_compsp.EQ.1.OR.write_compsp.EQ.3) &
       WRITE(10,fmt) zred,time,LOG10(mass+tiny_number),&
